...
design = read.csv("data/miRNA_editing_design_matrix.csv", row.names = 1)

mdl.mtx = model.matrix(~ Diagnosis + Age + Sex, design)


initial.dispersions = getBBdisperiosns(G.counts, A.counts, mdl.mtx)

MAP.dispersions = estMAPdisps(G.counts, A.counts, initial.dispersions)

ggplot(MAP.dispersions) +
  geom_point(aes(log_mean_total_counts, dispersion), colour = "black") +
  geom_point(aes(log_mean_total_counts, dispersion.map), colour = "blue") +
  geom_line(aes(log_mean_total_counts, dispersion.fit), colour = "red") 

final.results = getFinalResTable(G.counts, A.counts, mdl.mtx, MAP.dispersions)
